export default class Game {
	...
	static checkComplete(board) {
		...
		return board.flat().every((value, index) => {
			return value === Game.solvedBoard.flat()[index];
		});
	}
	...
	static move(row, col, value, game, setGame, moveCount, setMoveCount) {
		...
		if (this.moveUp(row, col, value, board) || this.moveRight(row, col, value, board) || this.moveDown(row, col, value, board) || this.moveLeft(row, col, value, board)) {
			// If any move was successful, check for completion
			if (this.checkComplete(board)) {
				setGame({ board, gameOver: true });
			} else {
				setGame({ board });
			}
